 ...
 *    it immediately.(quit, jobs, bg or fg)
 */
int builtin_cmd(char **argv)
{
    if(!strcmp(argv[0],"quit"))/* quit command */
        exit(0);
    if(!strcmp(argv[0],"jobs"))/* print joblist */
    {
        listjobs(jobs);
        return 1;
    }
    if(!strcmp(argv[0],"bg") || !strcmp(argv[0],"fg"))/* the exchange with the background and the foreground */
    {
        do_bgfg(argv);
        return 1;
    }
...
void waitfg(pid_t pid)
{
        ...
        while(pid==fgpid(jobs)){/* the state of the job of the pid is not FG */
	    sleep(1);
        }
...
{
    int status;
    pid_t pid;/* used to store the waitpid function return value */
    while((pid = waitpid(-1,&status,WNOHANG|WUNTRACED))>0)/* only once */
    {
        if(WIFSTOPPED(status))/* stopped , change the job's state */
        {
            getjobpid(jobs,pid)->state = ST;
            printf("Job [%d] (%d) stopped by signal %d\n",pid2jid(pid),pid,WSTOPSIG(status));
        }
        else if(WIFEXITED(status))/* exit , delete the job */
        {
            deletejob(jobs,pid);
        }
        else if(WIFSIGNALED(status))/* teminated by signal , delete the job */
        {
